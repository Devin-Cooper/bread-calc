...
export function mount(parent: HTMLElement, computed: ComputedRecipe, db: Database): void {
  ...
  parent.innerHTML = `
    <table class="chart-table">
      ...
      <tbody>
        ${computed.metrics.total_flour_g > 0 ? `<tr class="user-row"><td><strong>${escapeHtml(computed.recipe.name ?? "Your recipe")}</strong></td><td>—</td><td>${computed.metrics.total_flour_g}</td><td>${computed.metrics.total_water_g_nominal}</td><td>${(computed.hydration.nominal_pct ?? 0).toFixed(1)}%</td><td>${escapeHtml(computed.hydration.zone?.label ?? "—")}</td></tr>` : ""}
